...
import { MEDICINE_SCANNER_MAX_FILE_SIZE_BYTES } from './medicine-scanner-upload.options';

@Catch(MulterError)
export class MedicineScannerUploadExceptionFilter implements ExceptionFilter {
  ...
  private toHttpException(exception: MulterError): HttpException {
    if (exception.code === 'LIMIT_FILE_SIZE') {
      const maxMb = MEDICINE_SCANNER_MAX_FILE_SIZE_BYTES / (1024 * 1024);
